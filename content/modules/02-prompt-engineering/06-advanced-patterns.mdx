...
## Pattern 7: XML Tags for Structure

**What it is:** Wrapping distinct pieces of your prompt in XML tags so Claude can unambiguously distinguish inputs, instructions, examples, and context.

Claude is specifically trained to pay attention to XML tag structure. When your prompt has multiple parts — a document to analyze, a set of rules, a few-shot example block, the actual task — tags eliminate the risk of Claude conflating them.
...
<existing_code>
[paste the current file contents]
</existing_code>

<task>
Refactor the function in <existing_code> to use async/await.
Follow the <rules>.
</task>
```

Tags are especially powerful for long-context prompts. When you paste a 200-line document plus instructions, wrapping the document in `<document>...</document>` and the instructions in `<task>...</task>` makes the structure unambiguous.

### Long-context best practices

When you're including large reference material (specs, transcripts, long files) in a prompt:

1. **Put the longform data at the TOP** of the prompt, before the instructions. Claude follows instructions more reliably when the data comes first. This alone improves accuracy by a noticeable margin on retrieval-heavy tasks.
2. **Wrap each document in XML tags** with a clear name: `<spec>`, `<transcript>`, `<codebase>`.
3. **Ground responses in quotes.** Ask Claude to quote the relevant passages before answering: "Before answering, quote the 2-3 passages from `<spec>` that are most relevant."

```
<spec>
[large API specification]
</spec>
...
Using the <spec> above, identify every behavior the <existing_implementation> violates. For each violation, quote the exact line from <spec> that defines the required behavior.
```
