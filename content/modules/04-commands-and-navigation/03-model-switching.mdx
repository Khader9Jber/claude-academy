...
quiz:
  ...
  - id: "switch-2"
    question: "How does Opus 4.6 pricing compare to Sonnet 4.6 per token?"
    type: "multiple-choice"
    options:
      - "The same"
      - "Opus is roughly 5x more expensive per token (~$15/M input vs ~$3/M)"
      - "Opus is cheaper"
      - "Only Opus has a token cost"
    correct: 1
    explanation: "Approximate API pricing: Opus 4.6 is around $15 per million input tokens and $75 per million output, while Sonnet 4.6 is roughly $3/$15. Opus also tends to produce more reasoning tokens at higher effort, so a session on Opus drains your rolling budget noticeably faster. See current pricing at console.anthropic.com."
  - id: "switch-3"
...
```bash
/model opus      # Switch to Opus 4.6 (claude-opus-4-6)
/model sonnet    # Switch to Sonnet 4.6 (claude-sonnet-4-6) — default
/model haiku     # Switch to Haiku 4.5 (claude-haiku-4-5-20251001)
...
**Context windows by model:** Opus 4.6 has a 1M context window. Sonnet 4.6 and Haiku 4.5 both have 200K. If you're running out of context on Sonnet, `/model opus` gives you 5x more room at the cost of higher per-token pricing.
...
Approximate API pricing (see current rates at [console.anthropic.com](https://console.anthropic.com)):

| Model | Input (per M tokens) | Output (per M tokens) | Relative Speed |
|-------|---------------------|----------------------|----------------|
| Haiku 4.5 | ~$1 | ~$5 | Fastest |
| Sonnet 4.6 | ~$3 | ~$15 | Fast |
| Opus 4.6 | ~$15 | ~$75 | Slowest |

Opus also tends to produce more reasoning tokens at higher effort, so a session on Opus drains your rolling plan budget substantially faster than the same session on Sonnet.

**Smart switching** (Sonnet for most work, Haiku for mechanical edits, Opus for architecture and deep debugging) typically gets you more total throughput than staying on a single model — Haiku messages are cheap enough to offset the expensive Opus ones, and Sonnet handles the bulk well.
