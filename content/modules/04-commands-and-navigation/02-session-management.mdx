...
### 5. /rewind — Jump Back to a Checkpoint

Claude Code automatically creates a checkpoint **before each action** (file edit, command run, significant tool call). `/rewind` (or the Esc Esc shortcut) lets you jump back to any of those checkpoints.
...
**What happens:**
- A picker opens showing recent checkpoints
- Pick a checkpoint and choose what to roll back:
  - **Conversation only** — messages after the checkpoint disappear; files on disk stay as-is
  - **Code only** — files revert to the checkpoint state; conversation stays (Claude keeps the context of what it tried)
  - **Both** — conversation and files both return to the checkpoint state

**When to use /rewind:**
- Claude went in the wrong direction and you want to redirect (conversation-only, then guide it with what you learned)
- An edit broke something and you want the files reverted (code-only, then keep discussing)
- You want a full do-over of the last exchange (both)

Because checkpoints are automatic, you don't have to remember to `git stash` before risky changes — rewind is the native undo.
...
| **/rewind** | Rolls back (configurable) | **Rolls back (configurable)** | Preserved | Preserved | Same |

Of these, only `/rewind` can actually revert on-disk file changes — and only because Claude Code snapshots files automatically before each action. `/clear`, `/compact`, new sessions, and `/fork` never touch your filesystem.
...
Context window filling up?
  ...
  │
  └── Claude went in the wrong direction or broke something?
      └── /rewind (Esc Esc) — pick a checkpoint and choose conversation,
          code, or both
```
