...
quiz:
  ...
  - id: "skills-4"
    question: "What does disableAutoInvocation: true do?"
    type: "multiple-choice"
    options:
      - "Prevents the skill from working"
      - "Prevents automatic triggering — the skill must be invoked explicitly with /skillname"
      - "Disables the skill permanently"
      - "Stops notifications"
    correct: 1
    explanation: "disableAutoInvocation: true means the skill won't trigger automatically based on task matching. It can only be used when you explicitly invoke it with /skillname. This is useful for skills that should only run on demand — like deployment or migration skills."
---
