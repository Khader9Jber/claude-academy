...
quiz:
  - id: "mcp-db-1"
    ...
    options:
      ...
      - "claude mcp add postgres -e DATABASE_URL=postgresql://... --transport stdio npx -y @modelcontextprotocol/server-postgres"
      - "Connect via psql"
      - "Add a postgres section to package.json"
    correct: 1
    explanation: "Set up the PostgreSQL MCP server with: claude mcp add postgres -e DATABASE_URL=postgresql://... --transport stdio npx -y @modelcontextprotocol/server-postgres. This passes your connection string and starts the server. Claude can then query your database directly."
  - id: "mcp-db-2"
