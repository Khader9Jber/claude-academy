...
quiz:
  - id: "worktrees-1"
    ...
    options:
      ...
      - "Creates a new git worktree under .claude/worktrees/feature-auth/ with an auto-created branch and starts a Claude Code session there"
      - "Watches the auth directory"
      - "Switches to the auth branch"
    correct: 1
    explanation: "claude --worktree feature-auth creates a new git worktree, auto-creates a branch, and starts a Claude Code session inside the worktree. Your main working directory is untouched — all changes happen in the isolated worktree. Agents can also opt into the same isolation via isolation: worktree in their frontmatter."
  - id: "worktrees-2"
  ...
  - id: "worktrees-4"
    question: "When should you use a worktree instead of /rewind?"
    type: "multiple-choice"
    options:
      - "Always"
      - "When you need file-level isolation — worktrees create separate directories, while /rewind just restores a checkpoint in the same directory"
      - "Never — they're the same"
      - "Only for monorepos"
    correct: 1
    explanation: "Worktrees provide file-level isolation — a separate directory with its own file state, so multiple agents or experiments can run in parallel. /rewind restores a conversation or code checkpoint sequentially in the same directory. Use worktrees when experiments need to run side-by-side without contaminating each other. Use /rewind when you want to step back and try a different approach from a known-good checkpoint."
---
...
This:
1. Creates a worktree under `.claude/worktrees/feature-auth/`
...
Agents can request their own worktree without any CLI flag — add `isolation: worktree` to the agent's frontmatter and every invocation spins up a fresh worktree automatically:
...
```
1. Create:  claude --worktree my-experiment
...
| Aspect | Worktree | /rewind |
|---|---|---|
| What's isolated | Files (separate directory) | A checkpoint in the same session/directory |
| File changes | Completely isolated | Restored to a prior state |
| Git branch | New branch created | No branch change |
| Use case | Parallel experiments that shouldn't collide | Sequentially stepping back to retry |
| Persistence | Filesystem, survives restarts | Persists across sessions (recent snapshots) |
...
- You realize the current direction is wrong and want to **step back**
- The changes are small and you want to **retry** from a known-good state
- You don't need parallelism — just a single do-over

## Real-World Worktree Workflow

```bash
# Start a worktree for the refactoring
claude --worktree refactor-auth-service
